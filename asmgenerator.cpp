...
try
{
    ...
    if (listing && IsCode (section->type) && binary && listing.tellp () == position && position != std::streampos(-1) && !instruction.line) listing << '\n';
...
Code::Operand Generator::Context::GetAddress (const Code::Instruction& instruction) const
{
...
void Generator::Context::ApplyLocalDefinitions (const Code::Offset offset)
{
...
std::ostream& Generator::Context::WriteValue (std::ostream& stream, const Debugging::Value& value)
{
    switch (value.model)
    {
