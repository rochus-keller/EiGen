...
private:
    ...
    void ParseDirective (Instruction&, void (Parser::Context::*) (Expressions&) = nullptr);
    ...
    void ParseBinary (Expression&, void (Parser::Context::*) (Expression&));
...
void Parser::Context::EmitError (const Lexer::Token& token, const Message& message) const
{
	token.location.Emit (Diagnostics::Error, parser.diagnostics, message); throw Error {};
}

void Parser::Context::SkipCurrent ()
{
...
bool Parser::Context::SkipWhiteSpace ()
{
...
bool Parser::Context::IsCurrent (const Lexer::Symbol symbol) const
{
	return current.symbol == symbol;
}

void Parser::Context::Expect (const Lexer::Symbol symbol) const
{
	if (!IsCurrent (symbol)) EmitError (current, Format ("encountered %0 instead of %1", current, symbol));
}

void Parser::Context::Parse (const Lexer::Symbol symbol)
{
	Expect (symbol); SkipCurrent ();
}

void Parser::Context::Parse (Sections& sections)
{
...
void Parser::Context::Parse (Instructions& instructions)
{
    while (SkipWhiteSpace ()) {
...
void Parser::Context::ParseSection (Instructions& instructions)
{
...
void Parser::Context::DefineMacro ()
{
...
void Parser::Context::UndefineMacro ()
{
...
bool Parser::Context::ExpandMacro ()
{
...
void Parser::Context::Parse (Operand& operand, const Lexer::Symbol sentinel)
{
...
void Parser::Context::RepeatCode ()
{
...
void Parser::Context::Expand (const Tokens& tokens, const Operands& operands, Tokens::iterator position)
{
...
void Parser::Context::Expand (Lexer::Token& token, const Operands& operands)
{
...
bool Parser::Context::IsRecursive (const Lexer::Token& token, const Macros::const_iterator macro) const
{
...
const Parser::Context::Operand* Parser::Context::GetOperand (const String& string, String::size_type position, const Operands& operands)
{
...
void Parser::Context::Parse (Section& section, const Section::Type type)
{
	assert (IsSection (current.symbol)); SkipCurrent ();
	section.type = type; Parse (section.name); ParseAssembly (section);
}

void Parser::Context::Parse (Section::Name& name)
{
...
void Parser::Context::Parse (Instruction& instruction)
{
	if (IsCurrent (Lexer::Trace))
        return ParseDirective (instruction, &Parser::Context::ParseOptional);
	if (IsCurrent (Lexer::Assert) || IsCurrent (Lexer::Alignment) || IsCurrent (Lexer::Origin) || IsCurrent (Lexer::Align) ||
        ...
        return ParseDirective (instruction, &Parser::Context::ParseSingle);
	...
	if (instruction.mnemonic.empty ())
		if (IsCurrent (Lexer::Identifier))
            ...
            return ParseDirective (instruction, &Parser::Context::Parse);
...
void Parser::Context::ParseLineDirective ()
{
...
void Parser::Context::ParseDirective (Instruction& instruction, void (Parser::Context::*const parse) (Expressions&))
{
...
void Parser::Context::ParseSingle (Expressions& operands)
{
    operands.emplace_back ();
    Parse (operands.back());
}

void Parser::Context::ParseOptional (Expressions& operands)
{
	if (!IsCurrent (Lexer::Newline) && !IsCurrent (Lexer::Eof)) Parse (operands);
}

void Parser::Context::Parse (Expressions& operands)
{
	for (ParseSingle (operands); IsCurrent (Lexer::Comma); operands.back ().separated = true) SkipCurrent (), ParseSingle (operands);
}

void Parser::Context::Parse (Expressions& operands, const Lexer::Symbol sentinel)
{
	while (!IsCurrent (sentinel) && !IsCurrent (Lexer::Newline) && !IsCurrent (Lexer::Eof)) Parse (operands);
}

void Parser::Context::Parse (Expression& expression)
{
	ParseLogicalOr (expression);
    while (IsCurrent (Lexer::Colon)) ParseBinary (expression, &Parser::Context::ParseLogicalOr);
}

void Parser::Context::ParseLogicalOr (Expression& expression)
{
	ParseLogicalAnd (expression);
    while (IsCurrent (Lexer::LogicalOr)) ParseBinary (expression, &Parser::Context::ParseLogicalAnd);
}

void Parser::Context::ParseLogicalAnd (Expression& expression)
{
	ParseInclusiveOr (expression);
    while (IsCurrent (Lexer::LogicalAnd)) ParseBinary (expression, &Parser::Context::ParseInclusiveOr);
}

void Parser::Context::ParseInclusiveOr (Expression& expression)
{
	ParseExclusiveOr (expression);
    while (IsCurrent (Lexer::BitwiseOr)) ParseBinary (expression, &Parser::Context::ParseExclusiveOr);
}

void Parser::Context::ParseExclusiveOr (Expression& expression)
{
	ParseAnd (expression);
    while (IsCurrent (Lexer::BitwiseXor)) ParseBinary (expression, &Parser::Context::ParseAnd);
}

void Parser::Context::ParseAnd (Expression& expression)
{
	ParseIdentical (expression);
    while (IsCurrent (Lexer::BitwiseAnd)) ParseBinary (expression, &Parser::Context::ParseIdentical);
}

void Parser::Context::ParseIdentical (Expression& expression)
{
	ParseEquality (expression);
    while (IsCurrent (Lexer::Identical) || IsCurrent (Lexer::Unidentical)) ParseBinary (expression, &Parser::Context::ParseEquality);
}

void Parser::Context::ParseEquality (Expression& expression)
{
	ParseRelational (expression);
    while (IsCurrent (Lexer::Equal) || IsCurrent (Lexer::Unequal)) ParseBinary (expression, &Parser::Context::ParseRelational);
}

void Parser::Context::ParseRelational (Expression& expression)
{
	ParseShift (expression);
    while (IsCurrent (Lexer::Less) || IsCurrent (Lexer::LessEqual) || IsCurrent (Lexer::Greater) || IsCurrent (Lexer::GreaterEqual)) ParseBinary (expression, &Parser::Context::ParseShift);
}

void Parser::Context::ParseShift (Expression& expression)
{
	ParseAdditive (expression);
    while (IsCurrent (Lexer::LeftShift) || IsCurrent (Lexer::RightShift)) ParseBinary (expression, &Parser::Context::ParseAdditive);
}

void Parser::Context::ParseAdditive (Expression& expression)
{
	ParseMultiplicative (expression);
    if (IsCurrent (Lexer::Plus)) ParseBinary (expression, &Parser::Context::ParseAdditive);
    while (IsCurrent (Lexer::Plus) || IsCurrent (Lexer::Minus)) ParseBinary (expression, &Parser::Context::ParseMultiplicative);
}

void Parser::Context::ParseMultiplicative (Expression& expression)
{
	ParseUnary (expression);
    while (IsCurrent (Lexer::Times) || IsCurrent (Lexer::Slash) || IsCurrent (Lexer::Modulo)) ParseBinary (expression, &Parser::Context::ParseUnary);
}

void Parser::Context::ParseBinary (Expression& expression, void (Parser::Context::*const parse) (Expression&))
{
...
void Parser::Context::ParseUnary (Expression& expression)
{
...
void Parser::Context::ParsePostfix (Expression& expression)
{
...
void Parser::Context::ParsePrimary (Expression& expression)
{
...
void Parser::Context::ParseNumber (Expression& expression, const Base base)
{
	expression.model = Expression::Number; expression.value = Evaluate (current, base); SkipCurrent ();
}

void Parser::Context::ParseLiteral (Expression& expression, const Expression::Model model)
{
	expression.model = model; expression.string.swap (current.string); SkipCurrent ();
}

void Parser::Context::ParseFunctional (Expression& expression, const Expression::Operation operation)
{
...
void Parser::Context::ParseParenthesized (Expression& expression)
{
...
void Parser::Context::ParseNullary (Expression& expression)
{
	expression.model = Expression::NullaryOperation; expression.operation = current.symbol; SkipCurrent ();
}

Expression::Value Parser::Context::Evaluate (const Lexer::Token& literal, const Base base) const
{
