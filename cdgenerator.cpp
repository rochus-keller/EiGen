...
void Generator::Generate (const Section& section, std::ostream& stream) const
{
    ...
    if (!section.comment.empty ())
        stream << "; " << section.comment << '\n';
    if (IsAssembly (section))
        stream << Assembly::Lexer::Assembly << '\n';
    else
        Assembly::WriteIdentifier (stream << '.' << section.type << ' ', section.name) << '\n';
    if (section.required)
        stream << '\t' << Assembly::Lexer::Required << '\n';
    if (section.duplicable)
        stream << '\t' << Assembly::Lexer::Duplicable << '\n';
    if (section.replaceable)
        stream << '\t' << Assembly::Lexer::Replaceable << '\n';
    if (section.fixed)
        stream << '\t' << Assembly::Lexer::Origin << '\t' << section.origin << '\n';
    else if (section.alignment > 1)
        stream << '\t' << Assembly::Lexer::Alignment << '\t' << section.alignment << '\n';
    if (!section.group.empty ())
        Assembly::WriteIdentifier (stream << '\t' << Assembly::Lexer::Group << '\t', section.group) << '\n';
    if (!section.instructions.empty () && section.instructions.front ().comment.empty ())
        stream << '\n';
    for (auto& instruction: section.instructions)
        Generate (instruction, GetIndex (instruction, section.instructions) + 1, stream);
}
