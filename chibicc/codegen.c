...
static void gen_expr(Node *node) {
    // TODO println("  loc %s %d 0", file_name(node->tok->file->file_no), node->tok->line_no);
...
static void gen_stmt(Node *node) {
    ...
    case ND_EXPR_STMT:
        ...
        return;
    case ND_ASM:
        println("  #error: inline asm not yet supported"); //  node->asm_str
        return;
    }
...
static void emit_data(Obj *prog) {
    for (Obj *var = prog; var; var = var->next) {
        ...
        if (var->is_static)
            println("  #local var %s", var->name);
        else
            println("  #global var %s", var->name);
...
static void store_fp(int r, int offset, int sz) {
    println("  #store fp: reg=%d offset=%d size=%d", r, offset, sz);
}

static void store_gp(int r, int offset, int sz) {
    println("  #store gp: reg=%d offset=%d size=%d", r, offset, sz);
}
...
static void emit_text(Obj *prog) {
    for (Obj *fn = prog; fn; fn = fn->next) {
        ...
        if (fn->is_static)
            println("  #static function %s", fn->name);
        else
        ...
        // Prologue
        println("  #stack size: %d", fn->stack_size);
        println("  #alloca_bottom offset: %d", fn->alloca_bottom->offset);
        ...
        // Epilogue
        println("  #end function %s", fn->name);
    }
}
