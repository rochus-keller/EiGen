...
#define cc_stdcall_set  1
/** fastcall calling convention */
#define cc_fastcall_set 2
...
static ir_entity *get_function_entity(function_t *const function)
{
    ...
    ident    *id     = 0; // new_id_from_str(symbol->string);
    ident    *ld_id  = 0; //create_ld_ident((entity_t*)function);
	 ...
	 */
    ir_entity *irentity = 0; //ir_get_global(ld_id);
	if (irentity != NULL)
		goto entity_created;
...
//			return new_Const(get_mode_null(mode));
	...
	case DECLARATION_KIND_GLOBAL_VARIABLE: {
        ir_node *const addr = 0; //new_d_Address(dbgi, entity->variable.v.entity);
		return addr;
	}
...
//		store = get_store();
	...
	ir_node *result      = NULL;
	if (firm_builtin) {
        node = 0; //new_d_Builtin(dbgi, store, n_parameters, in, firm_builtin_kind,
                   //          ir_method_type);
        ...
        node = 0; //new_d_Call(dbgi, store, callee, n_parameters, in, ir_method_type);
		if (!(function_type->modifiers & DM_CONST)) {
...
static ir_node *statement_expression_to_ir(const statement_expression_t *expr)
{
	...
	if (!currently_reachable()) {
		...
		type_t *const type = skip_typeref(expr->base.type);
		if (!is_type_void(type))
            res = 0; //new_Bad(get_ir_mode_arithmetic(type));
	}
	return res;
}
...
//	POP_IRG();
    ...
    ir_entity *const entity = 0; //create_Block_entity(label->label->indirect_block);
//	return new_d_Address(dbgi, entity);
}
...
static ir_node *expression_to_control_flow(expression_t const *const expr, jump_target *const true_target, jump_target *const false_target)
{
	...
	default: {
        ...
        ir_node    *const right = 0; //new_Const(get_mode_null(get_irn_mode(val)));
                                   ...
                                   ir_linkage linkage)
{
    ...
    ident *ld_id = 0; //create_ld_ident(variable);
...
static ir_initializer_t *create_ir_initializer_string(initializer_t const *const init, type_t *type)
{
    ...
    ir_initializer_t *const irinit  = 0; //create_initializer_compound(arr_len);
...
static void create_local_static_variable(entity_t *entity)
{
    ...
    ident     *const id       = 0; //id_unique(entity->base.symbol->string);
    ir_entity *const irentity = 0; //new_global_entity(var_type, id, irtype,
                                    //              ir_visibility_local,
                                     //             IR_LINKAGE_DEFAULT);
//	set_entity_dbg_info(irentity, dbgi);
...
static jump_target *jump_to_label(label_t *const label)
{
    ...
    jump_target *tgt = 0; //&label->target;
//	jump_to_target(tgt);
...
//		ARR_APP1(ir_asm_constraint, constraints, constraint);
	...
	{
        size_t label_pos = 0; //pn_ASM_first_out + ARR_LEN(outs);
    ...
    ir_node *mem = 0; //needs_memory ? get_store() : new_NoMem();
    ...
    ident    *asm_text = 0; // new_id_from_str(statement->normalized_text->begin);
    ir_node  *node     = 0; //new_d_ASM(dbgi, mem, n_ins, ins, asm_text,
                             //      n_constraints, constraints,
                             //      n_clobbers, clobbers, flags);

	/* create output projs & connect them */
	if (needs_memory) {
        ir_node *const projm = 0; //new_Proj(node, mode_M, pn_ASM_M);
//		set_store(projm);
	}

    unsigned out_pos = 0; //pn_ASM_first_out;
	for (size_t i = 0, n = ARR_LEN(outs); i < n; ++i) {
...
{
	assert(!function->need_closure);
    ir_graph *irg             = 0; //current_ir_graph;
    ir_node  *args            = 0; //get_irg_args(irg);
	unsigned  n               = 0;
...
static void create_function(function_t *const function)
{
	...
	if (function->base.modifiers & DM_CONSTRUCTOR) {
        ir_type *segment = 0; //get_segment_type(IR_SEGMENT_CONSTRUCTORS);
		add_function_pointer(segment, function_entity, "constructor_ptr");
	}
	if (function->base.modifiers & DM_DESTRUCTOR) {
        ir_type *segment = 0; //get_segment_type(IR_SEGMENT_DESTRUCTORS);
