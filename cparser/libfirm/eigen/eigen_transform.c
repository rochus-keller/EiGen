...
/**
 * @file
 * @brief   code selection (transform FIRM into eigen FIRM)
 */
...
static ir_node *transform_const(ir_node *const node, ir_entity *const entity, ir_tarval *const value)
{
    ...
    return new_bd_eigen_Const(dbgi, block, entity, value);
}
...
{
    return transform_binop(node, new_bd_eigen_And);
}

static ir_node *gen_Or(ir_node *node)
{
    return transform_binop(node, new_bd_eigen_Or);
}

static ir_node *gen_Eor(ir_node *node)
{
    return transform_binop(node, new_bd_eigen_Xor);
}
...
#endif
    return transform_binop(node, new_bd_eigen_fDiv);
}
...
static ir_node *gen_Shl(ir_node *node)
{
    ...
    return transform_binop(node, new_bd_eigen_Shl);
}
...
static ir_node *gen_Shr(ir_node *node)
{
	...
	if (get_mode_modulo_shift(mode) != 32)
		panic("modulo shift!=32 not supported");
    return transform_binop(node, new_bd_eigen_Shr);
}
...
static ir_node *gen_Add(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return transform_binop(node, new_bd_eigen_fAdd);
	}
    ...
    return transform_binop(node, new_bd_eigen_Add);
}
...
static ir_node *gen_Sub(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return transform_binop(node, new_bd_eigen_fSub);
	}
    return transform_binop(node, new_bd_eigen_Sub);
}
...
static ir_node *gen_Mul(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return transform_binop(node, new_bd_eigen_fMul);
	}
    return transform_binop(node, new_bd_eigen_Mul);
}
...
static ir_node *gen_Minus(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return transform_unop(node, n_Minus_op, new_bd_eigen_fMinus);
	}
    return transform_unop(node, n_Minus_op, new_bd_eigen_Minus);
}

static ir_node *gen_Not(ir_node *node)
{
    return transform_unop(node, n_Not_op, new_bd_eigen_Not);
}
...
static ir_node *gen_Load(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return new_bd_eigen_fLoad(dbgi, new_block, new_mem, new_ptr);
	}
    return new_bd_eigen_Load(dbgi, new_block, new_mem, new_ptr);
}
...
static ir_node *gen_Store(ir_node *node)
{
	...
	if (mode_is_float(mode)) {
        return new_bd_eigen_fStore(dbgi, new_block, new_mem, new_ptr, new_val);
	}
    return new_bd_eigen_Store(dbgi, new_block, new_mem, new_ptr, new_val);
}
...
static ir_node *gen_Jmp(ir_node *node)
{
	ir_node  *new_block = be_transform_nodes_block(node);
	dbg_info *dbgi      = get_irn_dbg_info(node);
    return new_bd_eigen_Jmp(dbgi, new_block);
}
...
{
    be_start_out outs[N_eigen_REGISTERS] = { [REG_Sp] = BE_START_IGNORE };
...
{
    int                               p     = n_eigen_Return_first_result;
    ...
    in[n_eigen_Return_mem]   = be_transform_node(get_Return_mem(node));
    reqs[n_eigen_Return_mem] = arch_memory_req;

    in[n_eigen_Return_stack]   = get_irg_frame(irg);
    reqs[n_eigen_Return_stack] = &eigen_single_reg_req_sp_sp;
	...
	dbg_info *const dbgi  = get_irn_dbg_info(node);
	ir_node  *const block = be_transform_nodes_block(node);
    ir_node  *const ret   = new_bd_eigen_Return(dbgi, block, n_ins, in, reqs);
	return ret;
}
...
static ir_node *gen_Proj_Proj(ir_node *node)
{
	...
	if (is_Start(pred_pred)) {
		if (get_Proj_num(pred) == pn_Start_T_args) {
			...
			ir_graph *const irg = get_irn_irg(node);
            return be_get_Start_proj(irg, &eigen_registers[reg_params[arg_num]]);
		}
	}
...
static ir_node *gen_Proj_Load(ir_node *node)
{
	...
	case pn_Load_M:
        return be_new_Proj(new_load, pn_eigen_Load_M);
	case pn_Load_res:
        return be_new_Proj(new_load, pn_eigen_Load_res);
...
static ir_node *gen_Proj_Start(ir_node *node)
{
	...
	case pn_Start_P_frame_base:
        return be_get_Start_proj(irg, &eigen_registers[REG_Sp]);
	}
	panic("unexpected Start proj %u", pn);
}

static void eigen_register_transformers(void)
{
...
static void setup_calling_convention(ir_graph *irg)
{
    ...
    unsigned *allocatable_regs = rbitset_obstack_alloc(obst, N_eigen_REGISTERS);
    rbitset_set_all(allocatable_regs, N_eigen_REGISTERS);
...
/**
 * Transform generic IR-nodes into eigen machine instructions
 */
...
{
    FIRM_DBG_REGISTER(dbg, "firm.be.eigen.transform");
}
