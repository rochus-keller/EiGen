...
Converter::Context::Context (const Converter& c, const Information& i, const Source& s, Object::Binaries& b) :
    information(i), converter(c), source(s), binaries(b)
{
}

void Converter::Context::EmitError (const Message& message) const
{
	converter.diagnostics.Emit (Diagnostics::Error, source, {}, message); throw Error {};
}

void Converter::Context::Begin (const Binary::Name& name, const Binary::Required required, const Binary::Replaceable replaceable)
{
    binaries.emplace_back (Binary::Const, name, 1, required, true, replaceable);
    current = &binaries.back();
}

void Converter::Context::Alias (const Binary::Name& name)
{
	assert (current); current->aliases.emplace_back (name, current->bytes.size ());
}

void Converter::Context::Group (const Binary::Name& name)
{
	assert (current); assert (current->group.empty ()); current->group = name;
}

void Converter::Context::Require (const Binary::Name& name)
{
	assert (current); current->AddRequirement (name);
}

Object::Offset Converter::Context::Emit (const Size size)
{
	assert (current); const auto offset = current->bytes.size (); current->bytes.resize (offset + size); return offset;
}

void Converter::Context::Fixup (const Object::Offset offset, const Size size)
{
...
void Converter::Context::Emit (const Value::Unsigned value, const Size size)
{
...
void Converter::Context::Emit (const std::string& string, const Size size)
{
...
void Converter::Context::Emit (const Binary::Name& name, const Patch::Mode mode, const Patch::Displacement displacement, const Size size)
{
...
bool Converter::Context::IsDefined (const Binary::Name& name) const
{
	for (auto& binary: binaries) if (binary.name == name) return true; return false;
}

Converter::Context::Block::Block (const Entry& entry) :
...
Converter::Context::Block::Block (const Symbol& symbol) :
    lifetime(symbol.lifetime)
{
	symbols.push_back (&symbol);
}

void Converter::Context::Block::Insert (const Symbol& symbol)
{
    for (auto& block: blocks)
        if (block.lifetime.begin <= symbol.lifetime.begin && block.lifetime.end >= symbol.lifetime.end)
            return block.Insert (symbol);
    if (symbol.lifetime.begin == lifetime.begin && symbol.lifetime.end == lifetime.end)
        return symbols.push_back (&symbol);
    blocks.emplace_back (symbol);
    auto& block = blocks.back();
    if (block.lifetime.begin <= lifetime.begin && block.lifetime.end >= lifetime.end)
        std::swap (lifetime, block.lifetime),
                std::swap (symbols, block.symbols),
                std::swap (blocks, block.blocks);
}
