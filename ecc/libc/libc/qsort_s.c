...
/* Wrapper for _PDCLIB__memswp protects against multiple argument evaluation. */
static _PDCLIB_inline void _memswp( char * i, char * j, size_t size )
{
...
errno_t qsort_s( void * base, rsize_t nmemb, rsize_t size, int ( *compar )( const void *, const void *, void * ), void * context )
{
    char * i;
    char * j;
    _PDCLIB_size_t thresh = THRES * size;
    ...
    for ( ;; )
    {
        if ( ( size_t )( limit - base_ ) > thresh ) /* QSort for more than T elements. */
        {
            ...
            */
            _memswp( ( ( ( ( size_t )( limit - base_ ) ) / size ) / 2 ) * size + base_, base_, size );
            ...
            {
                _memswp( base_, j, size );
            }

            if ( compar( i, base_, context ) > 0 )
            {
                _memswp( i, base_, size );
            }
