...
void Tester$RunAll() {
    ...
    //Harness$run((const struct OBX$Array$1){5,0,OBX$FromUtf("Json",5,0)}, 100, 1); // fails verification
    Harness$run((const struct OBX$Array$1){7,0,OBX$FromUtf("Havlak",7,0)}, 10, 1);
    //Harness$run((const struct OBX$Array$1){4,0,OBX$FromUtf("CD2",4,0)}, 250, 2); // produces ERROR
