...
inline int Environment::Fputs (const std::string& string, const std::size_t stream)
{
    for (auto character: string) if (Fputc (character, stream) == -1) return -1; return 0;
}

inline int Environment::Getchar ()
{
    return Fgetc (stdin_);
}

inline int Environment::Putchar (const int character)
{
