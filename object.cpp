...
Pattern::Pattern (const Size size, const Endianness endianness)
{
    for (Size index = 0; index != size; ++index)
        operator += ({Mask::Offset (endianness == Endianness::Big ? size - index - 1 : index), 0xff});
}
...
Size GetSize (const Pattern& pattern)
{
...
std::istream& operator >> (std::istream& stream, Binary& binary)
{
    binary.bytes.clear (); binary.aliases.clear (); binary.links.clear ();
    Size size;
    if (stream >> binary.type >> size)
        binary.bytes.resize (size);
    ReadString (ReadOptions (stream, {&binary.required, &binary.duplicable,
                                      &binary.replaceable}, options, IsAlpha), binary.name, '"');
    ...
    for (auto& alias: binary.aliases)
        if (alias.offset > binary.bytes.size ())
            return stream.setstate (stream.failbit), stream;
    if (stream >> std::ws && stream.peek () == '"')
        ReadString (stream, binary.group, '"');
    else
        binary.group.clear ();
	ReadBool (stream, binary.fixed, placements, IsAlpha) >> (binary.fixed ? binary.origin : binary.alignment);
    if (IsType (binary.type) || !binary.fixed && !IsPowerOfTwo (binary.alignment))
        return stream.setstate (stream.failbit), stream;
    for (Offset offset; stream.good () && stream >> std::ws && stream.good () &&
                        std::isdigit (stream.peek ()) && stream >> offset;)
        do {
            if (offset < binary.bytes.size () && ReadByte (stream, binary.bytes[offset]))
                ++offset;
            else
                stream.setstate (stream.failbit);
        } while (stream.good () && std::isxdigit (stream.peek ()));
    while (stream.good () && stream >> std::ws && stream.good () && stream.peek () == '"') {
        binary.links.emplace_back ();
        stream >> binary.links.back();
    }
    for (auto& link: binary.links)
        for (auto& patch: link.patches)
            if (patch.offset + GetSize (patch.pattern) > binary.bytes.size ())
                return stream.setstate (stream.failbit), stream;
	return stream;
}

std::ostream& operator << (std::ostream& stream, const Binary& binary)
{
...
std::istream& operator >> (std::istream& stream, Binaries& binaries)
{
...
std::ostream& operator << (std::ostream& stream, const Binaries& binaries)
{
	for (auto& binary: binaries) stream << binary;
	return stream;
}

std::istream& operator >> (std::istream& stream, Alias& alias)
{
	return ReadString (stream >> alias.offset, alias.name, '"');
}

std::ostream& operator << (std::ostream& stream, const Alias& alias)
{
	return WriteString (stream << alias.offset << ' ', alias.name, '"');
}

std::istream& operator >> (std::istream& stream, Link& link)
{
...
std::istream& operator >> (std::istream& stream, Patch& patch)
{
	return stream >> patch.offset >> patch.mode >> patch.displacement >> patch.scale >> patch.pattern;
}

std::ostream& operator << (std::ostream& stream, const Patch& patch)
{
	return stream << patch.offset << ' ' << patch.mode << ' ' << patch.displacement << ' ' << patch.scale << ' ' << patch.pattern;
}
...
std::istream& operator >> (std::istream& stream, Mask& mask)
{
...
std::istream& operator >> (std::istream& stream, Patch::Mode& mode)
{
	return ReadEnum (stream, mode, modes, IsAlpha);
}

std::ostream& operator << (std::ostream& stream, const Patch::Mode mode)
{
	return WriteEnum (stream, mode, modes);
}

std::istream& operator >> (std::istream& stream, Section::Type& type)
{
	return ReadEnum (stream, type, types, IsAlpha);
}

std::ostream& operator << (std::ostream& stream, const Section::Type type)
{
