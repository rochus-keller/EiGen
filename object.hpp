...
namespace ECS
{
    ...
    namespace Object
    {
        ...
        inline Pattern::Pattern (const std::initializer_list<Mask> masks)
        {
            for (auto& mask: masks) operator += (mask);
        }

        inline bool IsCode (const Section::Type type)
        {
            return type < Section::Data;
        }

        inline bool IsData (const Section::Type type)
        {
            return type >= Section::Data && type < Section::TypeSection;
        }

        inline bool IsType (const Section::Type type)
        {
            return type == Section::TypeSection;
        }

        inline bool IsEmpty (const Pattern& pattern)
        {
            return pattern.size == 0;
        }

        inline bool IsAssembly (const Section& section)
        {
