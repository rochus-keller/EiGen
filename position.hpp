...
namespace ECS
{
    ...
    template <typename C>
    void Position::Advance (const C character)
    {
        if (character == '\n') ++line, newline += column, column = 1; else ++column;
    }

    template <typename C>
    void Position::Regress (const C character)
    {
        if (character == '\n') --line, --newline; else --column;
    }

    inline void Position::Override (const Line l)
    {
        line = l; column = 1;
    }

    template <typename C, typename T>
    std::basic_ostream<C, T>& Position::Indicate (std::basic_ostream<C, T>& stream) const
    {
    ...
    inline Line GetLine (const Position& position)
    {
        return position.line;
    }

    inline Column GetColumn (const Position& position)
    {
        return position.column;
    }

    inline Position GetPlain (const Position& position)
    {
        return {GetLine (position), GetColumn (position)};
    }

    template <typename C, typename T>
    std::basic_istream<C, T>& Advance (std::basic_istream<C, T>& stream, Position& position, C& character)
    {
        if (stream.get (character)) position.Advance (character != '\r' || stream.good () && stream.peek () == '\n' ? character : '\n'); return stream;
    }

    template <typename C, typename T>
    std::basic_istream<C, T>& Regress (std::basic_istream<C, T>& stream, Position& position, const C character)
    {
