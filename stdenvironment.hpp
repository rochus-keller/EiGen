...
inline int StandardEnvironment::Fflush (const std::size_t stream)
{
    return (stream >= stdout_ && stream <= stderr_ && (stream == stderr_ ? cerr : cout).flush ()) - 1;
}

inline int StandardEnvironment::Fgetc (const std::size_t stream)
{
    char character; return stream == stdin_ && cin.get (character) ? character : -1;
}

inline int StandardEnvironment::Fputc (const int character, const std::size_t stream)
{
