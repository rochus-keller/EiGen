...
namespace ECS
{
    ...
    inline UniversalCharacterName::UniversalCharacterName (const char32_t v) :
        value {v}
    {
    }

    template <typename C, typename T>
    std::basic_ostream<C, T>& operator << (std::basic_ostream<C, T>& stream, const UniversalCharacterName& character)
    {
    ...
    template <typename Value>
    Value Align (const Value offset, const Value alignment)
    {
    ...
    template <typename Value>
    inline Value ShiftLeft (const Value value, const Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    inline Value ShiftRight (const Value value, const Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
        return std::numeric_limits<Value>::is_signed && value < 0 ? ~(~value >> bits) : value >> bits;
    }
    ...
    template <typename Value>
    Value Scale (const Value value, const Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    Value Truncate (const Value value, Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <>
    inline double Truncate (const double value, const Bits bits)
    {
    ...
    template <typename Value>
    bool TruncatesPreserving (const Value value, const Bits bits)
    {
        return Truncate (value, bits) == value;
    }

    template <>
    inline bool TruncatesPreserving (const double value, const Bits bits)
    {
    ...
    template <typename Value>
    bool IsPowerOfTwo (const Value value)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    Bits CountOnes (const Value value)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    Bits CountTrailingZeros (const Value value)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    Value EncodeBits (Value value, Value mask)
    {
    ...
    template <typename Value>
    Value DecodeBits (Value code, Value mask)
    {
        //static_assert (std::numeric_limits<Value>::is_integer && !std::numeric_limits<Value>::is_signed);
    ...
    template <typename Value>
    Value RotateLeft (const Value value, const Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    Value RotateRight (const Value value, const Bits bits)
    {
        //static_assert (std::numeric_limits<Value>::is_integer);
    ...
    template <typename Value>
    inline bool CompareOrdered (const Value*const a, const Value*const b)
    {
        return std::less<const Value*> () (a, b);
    }

    template <typename Element, typename Container>
    bool InsertUnique (const Element& element, Container& container)
    {
    ...
    template <typename Element, typename Container>
    inline bool IsFirst (const Element& value, const Container& container)
    {
        return &value == &container.front ();
    }

    template <typename Element, typename Container>
    inline bool IsLast (const Element& value, const Container& container)
    {
        return &value == &container.back ();
    }

    template <typename Element, typename Container>
    inline bool IsMember (const Element& value, const Container& container)
    {
        return !CompareOrdered (&value, container.data ()) && CompareOrdered (&value, container.data () + container.size ());
    }

    template <typename Element, typename Container>
    inline std::size_t GetIndex (const Element& value, const Container& container)
    {
    ...
    template <typename Value>
    inline Restore<Value>::~Restore<Value> ()
    {
        value = previous;
    }

    template <typename Entry, typename Mnemonic> template <std::size_t entries>
    Lookup<Entry, Mnemonic>::Lookup (const Entry (&table)[entries])
    {
    ...
    template <typename Entry, typename Mnemonic> template <std::size_t entries>
    Lookup<Entry, Mnemonic>::Lookup (const Entry (&table)[entries], int)
    {
    ...
    template <typename Value>
    inline std::size_t Span<Value>::size () const noexcept
    {
        return last - first;
    }

    template <typename Value>
    inline Value& Span<Value>::operator [] (const std::size_t index) const noexcept
    {
        return first[index];
    }

    template <typename Value>
    inline Value* Span<Value>::begin () const noexcept
    {
        return first;
    }

    template <typename Value>
    inline Value* Span<Value>::end () const noexcept
    {
        return last;
    }

    template <typename C, typename T>
    std::basic_string<C, T> Lowercase (const std::basic_string<C, T>& string)
    {
        auto result = string; for (auto& character: result) character = std::tolower (character); return result;
    }

    template <typename C, typename T>
    std::basic_string<C, T> Uppercase (const std::basic_string<C, T>& string)
    {
        auto result = string; for (auto& character: result) character = std::toupper (character); return result;
    }

    template <typename Value, std::size_t size>
    bool FindEnum (const char identifier, Value& value, const char (&table)[size])
    {
    ...
    template <typename Value, std::size_t size>
    bool FindEnum (const std::string& identifier, Value& value, const char*const (&table)[size])
    {
        for (auto entry = table; entry != table + size; ++entry)
    ...
    template <typename C, typename T, typename Value, std::size_t size>
    bool FindSortedEnum (const std::basic_string<C, T>& identifier, Value& value, const char*const (&table)[size])
    {
    ...
    template <typename C, typename T>
    std::basic_istream<C, T>& ReadBool (std::basic_istream<C, T>& stream, bool& value, const char*const (&table)[2], bool (*const classify) (C))
    {
    ...
    template <typename C, typename T>
    inline std::basic_ostream<C, T>& WriteBool (std::basic_ostream<C, T>& stream, const bool value, const char*const (&table)[2])
    {
        return stream << table[value];
    }

    template <typename C, typename T, typename Value>
    std::basic_istream<C, T>& ReadFloat (std::basic_istream<C, T>& stream, Value& value, Value (*const convert) (const C*, C**))
    {
    ...
    template <typename C, typename T, std::size_t size>
    std::basic_istream<C, T>& ReadOptions (std::basic_istream<C, T>& stream, const std::initializer_list<bool*> options, const char*const (&table)[size], bool (*const classify) (C))
    {
    ...
    template <typename C, typename T, std::size_t size>
    std::basic_ostream<C, T>& WriteOptions (std::basic_ostream<C, T>& stream, const std::initializer_list<bool> options, const char*const (&table)[size])
    {
        for (auto& option: options) if (option) WriteEnum (stream << ' ', GetIndex (option, options), table); return stream;
    }

    template <typename C, typename T, typename Value, std::size_t size>
    std::basic_istream<C, T>& ReadEnum (std::basic_istream<C, T>& stream, Value& value, const char (&table)[size])
    {
        char identifier;
    ...
    template <typename C, typename T, typename Value, std::size_t size>
    std::basic_istream<C, T>& ReadEnum (std::basic_istream<C, T>& stream, Value& value, const char*const (&table)[size], bool (*const classify) (C))
    {
    ...
    template <typename C, typename T, typename Value, std::size_t size>
    std::basic_istream<C, T>& ReadSortedEnum (std::basic_istream<C, T>& stream, Value& value, const char*const (&table)[size], bool (*const classify) (C))
    {
        std::string identifier;
    ...
    template <typename C, typename T, typename Value, typename Identifier, std::size_t size>
    std::basic_ostream<C, T>& WriteEnum (std::basic_ostream<C, T>& stream, const Value value, const Identifier (&table)[size])
    {
        if (std::size_t (value) < size) stream << table[std::size_t (value)]; else stream.setstate (stream.failbit); return stream;
    }

    template <typename C, typename T>
    std::basic_istream<C, T>& ReadIdentifier (std::basic_istream<C, T>& stream, std::basic_string<C, T>& identifier, bool (*const classify) (C))
    {
    ...
    inline bool IsAlpha (const char character)
    {
        return std::isalpha (character);
    }

    inline bool IsAlnum (const char character)
    {
        return std::isalnum (character);
    }

    inline bool IsDotted (const char character)
    {
        return std::isalnum (character) || character == '.';
    }

    inline bool IsIdentifier (const char character)
    {
    ...
    inline bool IsAddress (const char character)
    {
        return IsIdentifier (character) || character == '?' || character == ':';
    }

    inline bool IsSpace (const char character)
    {
        return std::isspace (character);
    }

    template <typename C1, typename T1, typename C2, typename T2>
    std::basic_istream<C1, T1>& ReadEscaped (std::basic_istream<C1, T1>& stream, std::basic_string<C2, T2>& string, const char delimiter)
    {
    ...
    template <typename C1, typename T1, typename C2, typename T2>
    std::basic_ostream<C1, T1>& WriteEscaped (std::basic_ostream<C1, T1>& stream, const std::basic_string<C2, T2>& string, const char delimiter)
    {
    ...
    template <typename C1, typename T1, typename C2, typename T2>
    std::basic_istream<C1, T1>& ReadString (std::basic_istream<C1, T1>& stream, std::basic_string<C2, T2>& string, const char delimiter)
    {
        char character;
    ...
    template <typename C1, typename T1, typename C2, typename T2>
    inline std::basic_ostream<C1, T1>& WriteString (std::basic_ostream<C1, T1>& stream, const std::basic_string<C2, T2>& string, const char delimiter)
    {
        return WriteEscaped (stream << delimiter, string, delimiter) << delimiter;
    }

    template <typename C, typename T>
    std::basic_istream<C, T>& ReadByte (std::basic_istream<C, T>& stream, Byte& value)
    {
        char digit;
    ...
    template <typename C, typename T>
    std::basic_ostream<C, T>& WriteByte (std::basic_ostream<C, T>& stream, const Byte value)
    {
    ...
    template <typename C, typename T>
    std::basic_ostream<C, T>& WriteBytes (std::basic_ostream<C, T>& stream, const Span<const Byte> bytes)
    {
        for (auto byte: bytes) WriteByte (stream, byte); return stream;
    }

    template <typename C, typename T, typename Value>
    std::basic_istream<C, T>& ReadPrefixedValue (std::basic_istream<C, T>& stream, Value& value, const C*const prefix, const Value low, const Value high)
    {
    ...
    template <typename C, typename T, typename Value>
    std::basic_ostream<C, T>& WritePrefixedValue (std::basic_ostream<C, T>& stream, const Value value, const C*const prefix, const Value offset)
    {
    ...
    template <typename C, typename T>
    std::basic_ostream<C, T>& WriteAddress (std::basic_ostream<C, T>& stream, const char*const function, const std::basic_string<C, T>& address, const std::ptrdiff_t displacement, const Bits scale)
    {
    ...
    template <typename C, typename T, typename Value>
    std::basic_istream<C, T>& ReadOffset (std::basic_istream<C, T>& stream, Value& value)
    {
        const auto flags = stream.setf (stream.dec, stream.basefield);
    ...
    template <typename C, typename T, typename Value>
    std::basic_ostream<C, T>& WriteOffset (std::basic_ostream<C, T>& stream, const Value value)
    {
